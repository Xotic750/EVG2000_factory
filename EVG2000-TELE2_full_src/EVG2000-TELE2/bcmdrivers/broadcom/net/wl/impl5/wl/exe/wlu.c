 ...
 *
 * $Id: wlu.c,v 1.802.2.115 2009/06/19 19:57:33 Exp $
 */
...
static cmd_func_t wlu_ciswrite, wlu_cisdump;
...
static cmd_func_t wl_seq_start;
static cmd_func_t wl_seq_stop;

static cmd_func_t wl_lpphy_txiqcc;
static cmd_func_t wl_lpphy_txlocc;
static cmd_func_t wl_phytable;
static cmd_func_t wl_phy_pavars;
static cmd_func_t wl_phy_povars;
static cmd_func_t wl_phy_fem, wl_phy_maxpower, wl_antgain;
...
static int wlu_get(void *wl, int cmd, void *buf, int len);
static int wlu_set(void *wl, int cmd, void *buf, int len);
...
static int wl_assert_bypass(void *wl, cmd_t *cmd, char **argv);
...
static int cmd_pkt_list_num = 0;
static bool cmd_batching_mode = FALSE;
...
cmd_t wl_cmds[] = {
	...
	"Set per-tid ampdu regular rate retry limit; usage: wl ampdu_rr_retry_limit_tid "
	"<tid> [0~31]" },
	...
	"[amode open|shared|wpa|wpapsk|wpa2|wpa2psk|wpanone] [options]\n"
	"\tOptions:\n"
	...
	"Do ADC sample collect. Works for NPHY phyrevs >= 3.\n"
	"Optional parameters are:\n"
	"\tFile name to dump the sample buffer (binary format, default \"sample_collect.dat\")\n"
	"\tSample collect duration in us (default 60)\n"
	"\tCores to do sample collect, only if BW=40MHz (default both)\n"
	"The buffer returned has the format:\n"
	...
	" <tx|rx|txwithack|rxwithack> [ipg] [len] [nframes] [src]\n"
	...
	"\tvalue: 0 - Forward packet on match, discard on non-match (default).\n"
	"\t       1 - Discard packet on match, forward on non-match.\n" },
	...
	{ "mimo_ss_stf", wl_mimo_stf, WLC_GET_VAR, WLC_SET_VAR, "g/set SS STF mode.\n"
   ...
   {"led", wl_led, WLC_GET_LED, WLC_SET_LED, 
    "Wileless LED\n"
	"Usage: wl led pin func polarity {ledon_period ledoff_period}\n"
	"\tfunc selection:\n"
	"\t2  - activity\n"
	"\t4  - A radio\n"
	"\t5  - B radio\n"
...
#ifdef WLEXTLOG
	{ "extlog", wl_extlog, WLC_GET_VAR, -1, "get external logs\n"
...
#endif
	{ "assertlog", wl_assertlog, WLC_GET_VAR, -1, "get external assert logs\n"
	"\tUsage: wl assertlog\n"},
	{ "assert_bypass", wl_assert_bypass, WLC_GET_VAR, WLC_SET_VAR,
...
wlu_srvar(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((otp && wr) || co) {
		...
		if (!co)
			...
			ret = wl_iovar_set(wl, "cisvar", buf, cnt);
		return ret;
	}
...
wl_bss_max(void *wl, cmd_t *cmd, char **argv)
{
	...
	pval = (int *) buf;
	strcpy(buf, "cap");
	if ((error = wlu_get(wl, cmd->get, buf, WLC_IOCTL_MAXLEN)) < 0)
		return (error);
...
wl_chanim_state(void *wl, cmd_t *cmd, char **argv)
{
	...
	ret = wl_iovar_getbuf(wl, cmd->name, &chanspec, sizeof(chanspec),
	  buf, WLC_IOCTL_SMLEN);
	if (ret < 0)
		return ret;
	val = *(int*)buf;
...
wl_chanim_mode(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		if (cmd->get < 0)
			return -1;
		if ((ret = wl_iovar_getint(wl, cmd->name, &mode)) < 0)
			return ret;
		...
		mode = htod32(mode);
		return wl_iovar_setint(wl, cmd->name, mode);
	}
}
...
wl_parse_assoc_params(char **argv, wl_assoc_params_t *params)
{
	...
	while ((p = *argv) != NULL) {
		...
		if (opt == 'b' || !stricmp(key, "bssid")) {
			if (!wl_ether_atoe(valstr, &params->bssid)) {
				fprintf(stderr, "could not parse as an ethternet MAC address\n");
				err = BCME_BADARG;
				goto exit;
			}
		}
...
wl_wsec_test(void *wl, cmd_t *cmd, char **argv)
{
	...
	return wlu_set(wl, cmd->set, wsec_buf, len);
...
wl_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc == 0) {
		...
		err = wl_iovar_get(wl, "country", &cspec, sizeof(cspec));
	...
	if (cspec.rev == -1 && cspec.ccode[0] == '\0')
		err = wl_iovar_set(wl, "country", &cspec, WLC_CNTRY_BUF_SZ);
	else
		err = wl_iovar_set(wl, "country", &cspec, sizeof(cspec));
...
wl_sample_collect(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* 60us is roughly the max we can store. */
	collect.coll_us = 60;
	collect.cores = -1;
	collect.agc = FALSE;
	collect.filt = TRUE;
	...
	while (*argv) {
		...
		if (!strcmp(s, "-f"))
			...
			collect.agc = (atoi(argv[1]) == 1) ? TRUE : FALSE;
		else if (!strcmp(s, "-l"))
			collect.filt = (atoi(argv[1]) == 1) ? TRUE : FALSE;
		else
			goto exit;
	...
	ret = wl_iovar_getbuf(wl, "sample_collect", &collect, sizeof(wl_samplecollect_args_t),
...
wl_ampdu_activate_test(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((param = *++argv)) {
		x.val2 = atoi(param);
		printf("%d %d\n", x.val1, x.val2);
		err = wl_var_setbuf(wl, cmdname, &x, sizeof(x));
	}
	return err;
}
...
wl_ampdu_retry_limit_tid(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((param = *++argv)) {
		...
		err = wl_var_setbuf(wl, cmdname, &retry_limit, sizeof(retry_limit));
	} else {
		if ((err = wl_var_getbuf(wl, cmdname, &retry_limit, sizeof(retry_limit), &ptr)) < 0)
			return err;
...
wl_dpt_pmk(void *wl, cmd_t *cmd, char **argv)
{
	...
	return wl_var_setbuf(wl, cmdname, &psk, sizeof(wsec_pmk_t));
}
...
wl_dpt_fname(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((param = *++argv)) {
		...
		err = wl_var_setbuf(wl, cmdname, &fname, sizeof(fname));
	} else {
		if ((err = wl_var_getbuf(wl, cmdname, &fname, sizeof(fname), &ptr) < 0))
			return err;
...
wl_HCI_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	return wl_var_setbuf(wl, cmdname, &tbuf, 256);
}
...
wl_radar_args(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		printf("min_fm_lp %d max_span_lp %d min_deltat %d max_deltat %d\n",
		       rap->min_fm_lp, rap->max_span_lp, rap->min_deltat, rap->max_deltat);
		printf("autocorr 0x%x st_level_time 0x%x  t2_min %d fra_pulse_err %d\n",
		       rap->autocorr, rap->st_level_time, rap->t2_min, rap->fra_pulse_err);
		printf("npulses_fra %d npulses_stg2 %d npulses_stg3 %d percal_mask 0x%x quant %d\n",
			rap->npulses_fra, rap->npulses_stg2, rap->npulses_stg3, rap->percal_mask,
			rap->quant);
		printf("min_burst_intv_lp %d max_burst_intv_lp %d nskip_rst_lp %d max_pw_tol %d "
				"feature_mask 0x%x\n",
				rap->min_burst_intv_lp, rap->max_burst_intv_lp, rap->nskip_rst_lp,
				rap->max_pw_tol, rap->feature_mask);
	} else {
		/* Set */
		char *endptr;
		...
		*/
		...
		if (!*++argv)
			return -1;
		radar_args.max_span_lp = atoi(*argv);
		...
		if (!*++argv)
			return -1;
		radar_args.npulses_stg2 = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.npulses_stg3 = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.percal_mask = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.quant = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.min_burst_intv_lp = (uint32)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.max_burst_intv_lp = (uint32)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.nskip_rst_lp = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.max_pw_tol = (int)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
		if (!*++argv)
			return -1;
		radar_args.feature_mask = (uint16)strtol(*argv, &endptr, 0);
		if (*endptr != '\0') {	/* not all the value string was parsed by strtol */
			return -1;
		}
...
wlu_reg2args(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (get) {
		if (wl_var_getbuf(wl, cmd->name, var, sizeof(var), &ptr) < 0)
			return -1;

		printf("0x%x\n", dtoh32(*(int *)ptr));
	}
	else
		wl_var_setbuf(wl, cmd->name, &var, sizeof(var));
	return 0;
}
...
wl_auto_channel_sel(void *wl, cmd_t *cmd, char **argv)
{
	...
	*/
	...
	if ((ret = wlu_get(wl, WLC_GET_AP, &ap_mode, sizeof(ap_mode)))) {
		return ret;
	}
			...
			if(!ret) {
				...
				for(i = 0; i < 100; i++) {
					if(phytype == PHY_TYPE_N)
						...
						ret = wlu_get(wl, WLC_GET_CHANNEL_SEL, &chosen, sizeof(chosen));
					if(!ret)
						break;						
					sleep_ms(100);
				}				
...
wl_cac_prepare_g711_tspec(tspec_arg_t *tspec_arg, uint8 direction)
{
	...
	tspec_arg->mean_data_rate	= 83200;	/* 83.2kbps */
	tspec_arg->peak_data_rate 	= 83200;	/* 83.2kbps */
	tspec_arg->surplus_bw		= TSPEC_DEFAULT_SBW_FACTOR;
	tspec_arg->dialog_token		= TSPEC_DEFAULT_DIALOG_TOKEN;
	tspec_arg->min_phy_rate		= 6000000;
	tspec_arg->surplus_bw		= 0x2600;
}

static void
wl_cac_prepare_signal_tspec(tspec_arg_t *tspec_arg, uint8 direction)
{
	tspec_arg->version = TSPEC_ARG_VERSION;
	tspec_arg->length = sizeof(tspec_arg_t) - (2 * sizeof(uint16));
	...
	tspec_arg->mean_data_rate = 160;		/* 160 bps */
	tspec_arg->peak_data_rate = 160;		/* 160 bps */
	tspec_arg->inactivity_interval = 0;		/* disabled */
	tspec_arg->surplus_bw   = TSPEC_DEFAULT_SBW_FACTOR;
	tspec_arg->dialog_token = TSPEC_DEFAULT_DIALOG_TOKEN;
	tspec_arg->min_phy_rate	= 6000000;
	tspec_arg->surplus_bw	= 0x2600;
...
wl_cac(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!strcmp(arg1, "G711") ||
		...
		else if (!strcmp(arg2, "1") || !strcmp(arg2, "uplink"))
			direction = UPLINK_DIRECTION;
		else if (!strcmp(arg2, "2") || !strcmp(arg2, "downlink"))
			direction = DOWNLINK_DIRECTION;
		else {
			fprintf(stderr, "unknown direction option [ uplink (1) | downlink (2) ]\n");
			return BCME_BADARG;
		}
	}
	...
	if (cmd_type == 1) {
		if (!strcmp(arg1, "G711")) {
			wl_cac_prepare_g711_tspec(&tspec_arg, direction);
		} else if (!strcmp(arg1, "signal")) {
			wl_cac_prepare_signal_tspec(&tspec_arg, direction);
		} else {
			uint argc = 0;
...
wl_tslist_ea(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_var_getbuf(wl, cmd->name, &scb_val.ea, ETHER_ADDR_LEN, &ptr)) < 0)
		return err;
...
wl_tspec_ea(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_var_getbuf(wl, cmd->name, &tsea, sizeof(tspec_per_sta_arg_t), &ptr)) < 0)
		return err;
...
wl_lpphy_papdepstbl(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_iovar_get(wl, cmd->name, &eps_tbl, sizeof(eps_tbl))) < 0)
		return err;
...
wl_lpphy_txiqcc(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		if ((err = wl_iovar_get(wl, cmd->name, iqccValues, 2*sizeof(int32))) < 0)
			return err;
	...
	else
	{
		...
		if ((err = wl_var_setbuf(wl, cmd->name, iqccValues, 2*sizeof(int32))) < 0)
			return err;
	}
...
wl_lpphy_txlocc(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		if ((err = wl_iovar_get(wl, cmd->name, loccValues, sizeof(loccValues))) < 0)
			return err;
	...
	else
	{
		...
		if ((err = wl_var_setbuf(wl, "lpphy_txlocc", loccValues, 6*sizeof(int8))) < 0)
			return err;
	}
...
wl_phytable(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		if ((err = wl_var_getbuf(wl, cmd->name, tableInfo, 4*sizeof(int32), &ptr)) < 0)
			return err;
	...
	else
	{
		...
		if ((err = wl_var_setbuf(wl, cmd->name, tableInfo, 4*sizeof(int32))) < 0)
			return err;
	}
...
wl_phy_pavars(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* set */
		while (pav->phy_type != PHY_TYPE_NULL) {
			...
			if (found) {
				if ((err = wl_var_setbuf(wl, cmd->name, inpa,
		...
		while (pav->phy_type != PHY_TYPE_NULL) {
			...
			if ((err = wl_var_getbuf(wl, cmd->name, inpa,
...
wl_phy_povars(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* set */
		while (pov->phy_type != PHY_TYPE_NULL) {
			...
			if (found) {
				if ((err = wl_var_setbuf(wl, cmd->name, &inpo,
					sizeof(wl_po_t))) < 0) {
		...
		while (pov->phy_type != PHY_TYPE_NULL) {
			...
			if ((err = wl_var_getbuf(wl, cmd->name, &inpo, sizeof(povars_t),
				&ptr)) < 0) {
...
wl_phy_fem(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* write fem */
		...
		if (found) {
			if ((err = wl_var_setbuf(wl, "fem2g", &fem, sizeof(srom_fem_t)) < 0))
		...
		if (found) {
			if ((err = wl_var_setbuf(wl, "fem5g", &fem, sizeof(srom_fem_t)) < 0))
				printf("wl_phy_fem: fail to set fem5g\n");
			else
				printf("fem5g set\n");
		}
	} else {
		if ((err = wl_var_getbuf(wl, "fem2g", NULL, 0, (void**)&ptr) < 0)) {
		...
		if ((err = wl_var_getbuf(wl, "fem5g", NULL, 0, (void**)&ptr) < 0)) {
...
wl_phy_maxpower(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* write maxpower */
		...
		if ((err = wl_var_setbuf(wl, "maxpower", &maxp, 8 * sizeof(uint8)) < 0)) {
			printf("wl_phy_maxpower: fail to set\n");
			err = -1;
		}
	} else {
		if ((err = wl_var_getbuf(wl, "maxpower", NULL, 0, &ptr) < 0)) {
			printf("wl_phy_maxpower: fail to get maxpower\n");
			err = -1;
		}
		rmaxp = (uint8*)ptr;
		printf("maxp2ga0=0x%x\n", rmaxp[0]);
		printf("maxp2ga1=0x%x\n", rmaxp[1]);
		printf("maxp5ga0=0x%x\n", rmaxp[2]);
		printf("maxp5ga1=0x%x\n", rmaxp[3]);
		printf("maxp5gla0=0x%x\n", rmaxp[4]);
		printf("maxp5gla1=0x%x\n", rmaxp[5]);
		printf("maxp5gha0=0x%x\n", rmaxp[6]);
		printf("maxp5gha1=0x%x\n", rmaxp[7]);
	}
...
wl_antgain(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* write maxpower */
		...
		if ((err = wl_var_setbuf(wl, "antgain", &ag, 2 * sizeof(uint8)) < 0)) {
			printf("wl_antgain: fail to set\n");
			err = -1;
		}
	} else {
		if ((err = wl_var_getbuf(wl, "antgain", NULL, 0, &ptr) < 0)) {
...
wl_pkteng(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (strcmp(cmd->name, "pkteng_stop") == 0) {
		...
		} else if (strcmp(*argv, "rxwithack") == 0) {
...
wl_wowl_status(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_iovar_getint(wl, "wowl_status", &flags_prev)))
		return err;
...
wl_rifs_advert(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*argv) {
		if ((err = wl_iovar_getint(wl, val_name, (int*)&rifs_advert)) < 0)
			return err;
	...
	if ((err = wl_iovar_setint(wl, val_name, (int)rifs_advert)) < 0)
...
{
	if (!batch_in_client) {
		return wl_iovar_setbuf(wl, "seq_start", NULL, 0, buf, WLC_IOCTL_MAXLEN);
	}
	else {
		if (cmd_batching_mode) {
...
wl_seq_stop(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!batch_in_client) {
		return wl_iovar_setbuf(wl, "seq_stop", NULL, 0, buf, WLC_IOCTL_MAXLEN);
	}
	else {
		if (!cmd_batching_mode) {
		...
		ret = wl_iovar_setbuf(wl, "seq_start", NULL, 0, buf, WLC_IOCTL_MAXLEN);
		if (ret) {
		...
		ret = wl_iovar_setbuf(wl, "seq_stop", NULL, 0, buf, WLC_IOCTL_MAXLEN);
		if (ret) {
...
wl_keep_alive(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (NULL == *++argv) {
	   ...
	   */
		if ((rc = wl_var_getbuf(wl, cmd->name, NULL, 0, &ptr)) < 0)
			return rc;
...
wl_pkt_filter_list(void *wl, cmd_t *cmd, char **argv)
{
	...
	*/
	if ((rc = wl_var_getbuf(wl, cmd->name, &enable, sizeof(enable), &ptr)) < 0)
		return rc;
...
wl_pkt_filter_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* Get debug stats. */
	if ((rc = wl_var_getbuf(wl, cmd->name, &id, sizeof(id), &ptr)) < 0)
		return rc;
...
wl_obss_coex_action(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!options)
		return -1;
	err = wl_var_setbuf(wl, cmd->name, &var, (sizeof(wl_action_obss_coex_req_t) +
...
wl_srchmem(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((argc == 1)) {
		...
		ret = wl_iovar_getbuf(wl, iovar, &x, sizeof(x), buf, WLC_IOCTL_SMLEN);
		if (ret < 0) {
		...
		ret = wl_iovar_setbuf(wl, iovar, &x, sizeof(x), buf, WLC_IOCTL_MAXLEN);
		if (ret < 0) {
...
#ifdef DSLCPE
   if ( argv[4] != NULL ) {
      info.on_period  = (uint32) strtol(argv[4], NULL, 0);
      if ( info.on_period == 0 ) {
	  	printf("Error on_period cannot be 0\n" );
		return -1;
      	}
    	
       if ( argv[5] != NULL ) {
           info.off_period  = (uint32) strtol(argv[5], NULL, 0);
           if ( info.off_period == 0 ) {
	            printf("Error off_period cannot be 0\n" );
		     return -1;
      	    }
       }
   }
...
wl_readscb(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_var_getbuf(wl, cmd_arg, NULL, 0, &p)) < 0) {
		return err;
	}
...
wl_extlog(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((err = wl_var_getbuf(wl, cmd->name, &r_args, sizeof(wlc_extlog_req_t), &ptr)) < 0)
			return err;
	...
	for (i = 0; i < (int)results->num; i++) {
		...
		switch (extlog_fmt_str[j].arg_type) {
			case LOG_ARGTYPE_NULL:
				printf(extlog_fmt_str[j].fmt_str);
				break;
...
wl_extlog_cfg(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc == 1) {
		...
		wl_var_setbuf(wl, cmd->name, &w_cfg, sizeof(wlc_extlog_cfg_t));
	}
	else {
...
static int
wl_assert_bypass(void *wl, cmd_t *cmd, char **argv)
{
	int err;
	int bypass;
	char *val_name;
	...
	if (!*argv) {
		if ((err = wl_iovar_getint(wl, val_name, (int*)&bypass)) < 0)
			return err;
	...
	bypass = (atoi(*argv) ? 1 : 0);
	if (bypass != 0 && bypass != 1)
		return USAGE_ERROR;

	if ((err = wl_iovar_setint(wl, val_name, (int)bypass)) < 0)
		printf("Set assert_bypass error %d\n", err);
...
wl_ledbh(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {	/* set */
		...
		if ((err = wl_var_getbuf(wl, cmd->name, (void*)&led,
		...
		if ((err = wl_var_setbuf(wl, cmd->name, (void*)&led,
...
wl_led_blink_sync(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc == 2) {
		err = wl_var_getbuf(wl, cmd->name, (void*)in_arg, sizeof(int), &ptr);
		if (err < 0)
			return err;
		...
		wl_var_setbuf(wl, cmd->name, in_arg, sizeof(in_arg));
	}
	else {
		fprintf(stderr, "illegal command!\n");
		return -1;
