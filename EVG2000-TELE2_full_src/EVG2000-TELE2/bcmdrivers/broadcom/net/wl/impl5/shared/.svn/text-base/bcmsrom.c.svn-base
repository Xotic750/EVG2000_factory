 ...
 *
 * $Id: bcmsrom.c,v 1.166.2.101 2009/06/30 01:12:13 Exp $
 */
...
#if defined(BCMUSBDEV) || defined(DSLCPE_SDIO)
#include <sbsdio.h>
#include <sbhnddma.h>
...
#if defined(WLTEST) || defined(USB4322) || (defined(USB4319) && \
...
#if defined(DSLCPE) && defined(DSLCPE_WOMBO)
int wl_srom_present = 0; /* wombo board don't have srom */
...
#else /* def BCMUSBDEV || def DSLCPE_SDIODEV */
#ifdef DSLCPE_SDIO
	else if (BUSTYPE(bustype) == SDIO_BUS) {
		crc_range = SROM_MAX;
	}
#endif /* def DSLCPE_SDIO */
	else {
...
static char BCMNMIATTACHDATA(vstr_wpsled)[] = "wpsled=%d";
...
static char BCMNMIATTACHDATA(vstr_cddpo)[] = "cddpo=0x%x";
static char BCMNMIATTACHDATA(vstr_stbcpo)[] = "stbcpo=0x%x";
static char BCMNMIATTACHDATA(vstr_bw40po)[] = "bw40po=0x%x";
static char BCMNMIATTACHDATA(vstr_bwduppo)[] = "bwduppo=0x%x";
			...
			case CISTPL_BRCM_HNBU:
				...
				case HNBU_CUSTOM1:
					varbuf_append(&b, vstr_custom, 1, ((cis[i + 4] << 24) +
						...
						cis[i + 1]));
					break;
#if defined(DSLCPE_SDIO) || defined(BCMCCISSR3)
	...
	bool check_crc)
{
	...
	if (!sih->wl_srom_present) {
		...
		read_sromfile(sih->wl_srom_sw_map,  buf, wordoff*2, nwords*2);
		return err;		
	}	
#endif
...
BCMATTACHFN(initvars_srom_pci)(si_t *sih, void *curmap, char **vars, uint *count)
{
	...
	/* Use OTP if SPROM not available */
		...
		/* OTP only contain SROM rev8 for now */
		sromrev = srom[SROM4_CRCREV] & 0xff;
	}
#endif /* defined(BCMNVRAMW) || defined(BCMNVRAMR) */
	else {
...
BCMATTACHFN(initvars_srom_si)(si_t *sih, osl_t *osh, void *curmap, char **vars, uint *varsz)
{
	...
	*varsz = srom_vars_len(defaultsromvars_4319usb);
	*vars = MALLOC(osh, (*varsz + 1));
	...
	bzero(*vars, (*varsz + 1));
	bcopy(defaultsromvars_4319usb, *vars, *varsz);
	*varsz  = *varsz + 1;
#else
	*vars = _vars;
	*varsz = _varsz;
