...
int gLedCount = 0;
unsigned int gLedRunningCounter = 0;
...
void ledOn(PLED_INFO pLed)
{
    if( pLed->ledMask )
    {
        if (pLed->ledSerial) {
            ...
            *gpio_dir_reg |= pLed->ledMask;         // turn on the direction bit in case was turned off by some one
            if( pLed->ledActiveLow )
                *gpio_io_reg  &= ~pLed->ledMask;    // turn on the led
            else
                *gpio_io_reg  |= pLed->ledMask;     // turn on the led
        }
...
void ledOff(PLED_INFO pLed)
{
    if( pLed->ledMask )
    {
        if (pLed->ledSerial) {
            ...
            *gpio_dir_reg |= pLed->ledMask;         // turn on the direction bit in case was turned off by some one
            if( pLed->ledActiveLow )
                *gpio_io_reg  |= pLed->ledMask;     // turn off the led
            else
                *gpio_io_reg  &= ~pLed->ledMask;    // turn off the led
        }
...
void ledOnFail(PLED_INFO pLed)
{
    if( pLed->ledMaskFail )
    {
        if (pLed->ledSerialFail) {
            ...
            if (pLed->ledMaskFailHigh)
            {
                gpio_dir_reg = &GPIO->GPIODir_high;
                gpio_io_reg  = &GPIO->GPIOio_high;
            }
            *gpio_dir_reg |= pLed->ledMaskFail;     // turn on the direction bit in case was turned off by some one
            if( pLed->ledActiveLowFail )
                *gpio_io_reg &= ~pLed->ledMaskFail;// turn on the led
            else
                *gpio_io_reg |= pLed->ledMaskFail; // turn on the led
        }
...
void ledOffFail(PLED_INFO pLed)
{
    if( pLed->ledMaskFail )
    {
        if (pLed->ledSerialFail) {
            ...
            *gpio_dir_reg |= pLed->ledMaskFail;     // turn on the direction bit in case was turned off by some one
            if( pLed->ledActiveLowFail )
                *gpio_io_reg |= pLed->ledMaskFail; // turn off the led
            else
                *gpio_io_reg &= ~pLed->ledMaskFail;// turn off the led
        }
        pLed->ledState = pLed->savedLedState = kLedStateOff;
...
BOARD_LED_STATE ledToggle(PLED_INFO pLed)
{
    ...
    if (pLed->ledSerial) {
        ...
        *gpio_dir_reg |= pLed->ledMask;         // turn on the direction bit in case was turned off by some one
        if (*gpio_io_reg & pLed->ledMask) {
            *gpio_io_reg &= ~(pLed->ledMask);
            ledState = pLed->ledActiveLow ? kLedStateOn : kLedStateOff;
        }
        else {
            *gpio_io_reg |= pLed->ledMask;
            ...
            for( pLed = gLed; pLed != pCurLed; pLed++ )
            {
                   ...
                   (pLed->ledMaskFail && pLed->ledMaskFail==p1->ledMaskFail))
                {
                    ...
                    if( p1->ledMask )
                    {
                        ...
                        pLed->ledMaskHigh = p1->ledMaskHigh;
                    ...
                    if( p1->ledMaskFail )
                    {
                        ...
                        pLed->ledMaskFailHigh = p1->ledMaskFailHigh;
...
void boardLedCtrl(BOARD_LED_NAME ledName, BOARD_LED_STATE ledState)
{
    ...
    switch (ledState)
    {
        ...
        case kLedStateUserWpsInProgress:          /* 200ms on, 100ms off */
            ledInfoPtr->blinkCountDown = kSlowBlinkCount;
            ledInfoPtr->ledState = kLedStateUserWpsInProgress;
            ledInfoPtr->savedLedState = kLedStateUserWpsInProgress;
            ledTimerStart();
            break;             
            
        case kLedStateUserWpsError:               /* 100ms on, 100ms off */
            ledInfoPtr->blinkCountDown = kFastBlinkCount;
            ledInfoPtr->ledState = kLedStateUserWpsError;
            ledInfoPtr->savedLedState = kLedStateUserWpsError;
            ledTimerStart();
            break;        
            
        case kLedStateUserWpsSessionOverLap:      /* 100ms on, 100ms off, 5 times, off for 500ms */
            ledInfoPtr->blinkCountDown = kFastBlinkCount;
            ledInfoPtr->ledState = kLedStateUserWpsSessionOverLap;
            ledInfoPtr->savedLedState = kLedStateUserWpsSessionOverLap;
            ledTimerStart();
            break;        
