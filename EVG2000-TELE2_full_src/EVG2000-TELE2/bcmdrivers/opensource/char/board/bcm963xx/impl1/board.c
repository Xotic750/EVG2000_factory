                        ...
                        unsigned int command, unsigned long arg )
{
    ...
    switch (command) 
    {
        ...
        case BOARD_IOCTL_SET_BASE_MAC_ADDRESS:
        {
            ...
            {
                readNvramData(&nvramData);
                __copy_from_user((char*)nvramData.ucaBaseMacAddr,ctrlParms.string, NVRAM_MAC_ADDRESS_LEN);
        ...
        case BOARD_IOCTL_GET_GPIO:
            if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
                ...
                ctrlParms.result = GetGpio(ctrlParms.strLen);
                __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
                ret = 0;
            } 
            else {
...
static void sesLed_ctrl(int action)
{
    ...
    switch (action) 
    {
        case SES_LED_ON:
            ...
            break;
        case SES_LED_BLINK:
            //printk("SES: led blink, blinktype=%d\n", blinktype);
            if(blinktype)
            	led = blinktype;
            else
...
int GetGpio(int gpio_bit)
{
    ...
    if( gpio_bit >= 32 )
    {
        gpio_mask = GPIO_NUM_TO_MASK_HIGH(gpio_bit);
