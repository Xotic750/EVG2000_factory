...
int br_mld_set_port_snooping(const char *br, const char *port, const char *addr)
{
	...
	return port_set(br, port, "port_snooping", dest, BRCTL_MLD_SET_PORT_SNOOPING);
}
...
int br_mld_clear_port_snooping(const char *br, const char *port, const char *addr)
{
	...
	for (i=0; i < 8; i++)
	    dest[i+21] = iaddr[i];
	return port_set(br, port, "port_snooping", dest, BRCTL_MLD_CLEAR_PORT_SNOOPING);
}
