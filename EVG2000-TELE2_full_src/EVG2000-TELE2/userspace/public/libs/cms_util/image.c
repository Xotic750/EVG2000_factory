...
#include "bcmTag.h" /* in shared/opensource/include/bcm963xx, for FILE_TAG and Crc32_table */
#undef BCMTAG_EXE_USE


static UINT32 getCrc32(UINT8 *pdata, UINT32 size, UINT32 crc);
...
// Returns      : crc.
****************************************************************************/
UINT32 getCrc32(UINT8 *pdata, UINT32 size, UINT32 crc) {
   while (size-- > 0)
...
CmsRet verifyBroadcomFileTag(FILE_TAG *pTag)
{
    ...
    crc = getCrc32((UINT8 *) pTag, (UINT32)TAG_LEN-TOKEN_LEN, crc);
    if (crc != tokenCrc)
    {
    ...
    crc = CRC32_INIT_VALUE;
    crc = getCrc32(((UINT8 *)pTag + TAG_LEN), (UINT32) totalImageSize, crc);      
    if (crc != imageCrc)
    {
        cmsLog_debug("image crc failed, this is not a valid broadcom image");
        cmsLog_debug("calculated crc=0x%x imageCrc=0x%x", crc, imageCrc); 
        return CMSRET_INVALID_IMAGE;
    }
    ...
    cmsLog_debug("tarVer=%d validRange=%d-%d", tagVer, curVer, tagVerLast);
...
#ifdef CONVERT_PSI_TO_MDM
   else if ((strncmp(buf, "<psitree>", 9)) == 0)
   {
      likely = TRUE;
   }
#endif
...
CmsImageFormat cmsImg_validateImage(const char *imageBuf, UINT32 imageLen, void *msgHandle)
{
...
#ifdef CONVERT_PSI_TO_MDM
      else if (ret == CMSRET_CONFIG_PSI)
      {
         cmsLog_debug("PSI config file detected");
         return CMS_IMAGE_FORMAT_XML_CFG;
      }
#endif
   ...
   if (verifyBroadcomFileTag((FILE_TAG *) imageBuf) == CMSRET_SUCCESS)
   {
      UINT32 maxLen;
   ...
   else
   {
      ...
      UINT32 imageCrc;
      UINT8 *crcPtr = (UINT8 *) (imageBuf + (imageLen - TOKEN_LEN));
      ...
      crc = getCrc32((unsigned char *) imageBuf, imageLen - TOKEN_LEN, crc);      
      if (crc == imageCrc)
      {
          UINT32 maxLen;
