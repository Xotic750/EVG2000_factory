...
int session_connect(struct session *ses)
{
     ...
     * PPPoE1 can receive packets for PPPoE2, and vice versa.
     */
    while (1) {
        unsigned int from_len = sizeof(struct sockaddr_ll);
        int len;
        if (disc_sock >= 0) {
            len = recvfrom( disc_sock, rcv_packet.buf, 1500, MSG_DONTWAIT,
	    	          (struct sockaddr*)&rcv_packet.addr, &from_len);
            if (len > 0) {
                poe_info(ses, "got %d bytes in PPPoE discovery socket", len);
            } else
                break;
        }
    }
