...
#include "dhcpd.h"
#include "options.h"
#if defined(U12L112_Fullrate)
#include "dhcpc.h" //Silver add for test 2010/1/7
#endif
void init_header(struct dhcpMessage *packet, char type)
{
	...
	case DHCPRELEASE:
	case DHCPINFORM:
	case DHCPDECLINE: /* fiji wklin added, 08/10/2007 */
		   ...
		   u_int32_t dest_ip, int dest_port)
{
    ...
    /* wklin added start for test, 01/07/2010 */
#if defined(U12L112_Fullrate)
    if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, client_config.interface,
            strlen(client_config.interface)+1) == -1)
        perror(setsockopt);
#endif
    /* wklin added end for test 01/07/2010 */
