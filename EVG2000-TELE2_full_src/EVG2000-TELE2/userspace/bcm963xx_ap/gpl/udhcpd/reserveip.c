...
#define BUFFER_SIZE     2*1024
    ...
    char resrvfile[50] = "";
    if (whichlan == 0)
        strcpy(resrvfile, "/var/udhcpd/udhcpd_resrv.conf");
    else if (whichlan == 1)
        strcpy(resrvfile, "/var/udhcpd/udhcpd_resrv_br1.conf");
    else if (whichlan == 2)
        strcpy(resrvfile, "/var/udhcpd/udhcpd_resrv_br2.conf");
    else if (whichlan == 3)
...
void set_STB_reserved_ip(u_int8_t *chaddr)
{
    int i;

    for (i=0; i<4; i++)
    {
        if ( memcmp(chaddr, resrvMacAddr[i], 6) == 0 )
            return;
    }
    for (i=0; i<4; i++)
    {
        if ( strcmp(resrvMacAddr[i], "00:00:00:00:00:00") != 0 )
        {
            memcpy(resrvMacAddr[i], chaddr, 6);
            return;
        }
    }
    return;
