...
{
    unsigned char option[2 + 64];
    LOG(LOG_INFO, "add_hostname_option %d size %d name %s", DHCP_HOST_NAME,strlen(hostname)+1,hostname);

    option[OPT_CODE] = DHCP_HOST_NAME;
    option[OPT_LEN] = strlen(hostname)+1;

    memcpy(option + 2, hostname, strlen(hostname)+1);
    return add_option_string(optionptr, option);
}
#endif
