...
extern unsigned int txRelay;
extern unsigned int txByte;
void sigusr1_handler(int sig)
{
    FILE *fp = NULL;
    fp = fopen("/var/pppRelayStat", "w+");
    if (fp)
    {
        fprintf(fp, "rxRelay=%u\n", rxRelay);
        fprintf(fp, "rxByte=%u\n", rxByte);
        fprintf(fp, "txRelay=%u\n", txRelay);
        fprintf(fp, "txByte=%u\n", txByte);
        fclose(fp);
    }
}
...
relayLoop()
{
	...
	RELAY_DEBUG("Zz [fun=%s,%d],dissock[%d]=%d\n",__FUNCTION__,__LINE__,i,sock);/*Zz*/
	if (sock > maxFD) maxFD = sock;
	FD_SET(sock, &readable);
	sock = Interfaces[i].sessionSock;
	RELAY_DEBUG("Zz [fun=%s,%d],sesssock[%d]=%d\n",__FUNCTION__,__LINE__,i,sock);/*Zz*/
	if (sock > maxFD) maxFD = sock;
	FD_SET(sock, &readable);
	if (CleanPipe[0] > maxFD) maxFD = CleanPipe[0];
	FD_SET(CleanPipe[0], &readable);
	RELAY_DEBUG("Zz [fun=%s,%d],CleanPipefd:%d\n",__FUNCTION__,__LINE__,CleanPipe[0]);/*Zz*/
    }
    maxFD++;
    RELAY_DEBUG("Zz [fun=%s,%d],maxFD=%d\n",__FUNCTION__,__LINE__,maxFD);/*Zz*/
    for(;;) {
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],r=%d\n",__FUNCTION__,__LINE__,r);/*Zz*/
	/* Handle session packets first */
	for (i=0; i<NumInterfaces; i++) {
	    RELAY_DEBUG("Zz [fun=%s,%d],num=%d\n",__FUNCTION__,__LINE__,i);/*Zz*/
...
relayGotDiscoveryPacket(PPPoEInterface const *iface)
{
	...
	RELAY_DEBUG("Zz [fun=%s,%d],packet type error!\n",__FUNCTION__,__LINE__);/*Zz*/
	return;
    }

    /* Validate length */
    if (ntohs(packet.length) + HDR_SIZE > size) {
    RELAY_DEBUG("Zz [fun=%s,%d],length error!\n",__FUNCTION__,__LINE__);/*Zz*/
...
relayGotSessionPacket(PPPoEInterface const *iface)
{
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],mac not match!\n",__FUNCTION__,__LINE__);/*Zz*/
	return;
    }
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],go to relay!\n",__FUNCTION__,__LINE__);/*Zz*/
    /* Relay it */
    ses = sh->ses;
    ses->epoch = Epoch;
    sh = sh->peer;
