...
#if 0 
#define RELAY_DEBUG printf
#else
#define RELAY_DEBUG(format, args...) {}
#endif
...
RELAY_DEBUG("Zz [fun=%s,%d],USE_LINUX_PACKET\n",__FUNCTION__,__LINE__);/*Foxconn modify,Zz Shan 01/03/2008*/
    ...
    if (hwaddr) {
	RELAY_DEBUG("Zz [fun=%s,%d],in hwaddr\n",__FUNCTION__,__LINE__);/*Zz*/
	strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
	RELAY_DEBUG("Zz [fun=%s,%d],ifname:%s\n",__FUNCTION__,__LINE__,ifr.ifr_name);/*Zz*/
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],hwaddr=%02x:%02x:%02x:%02x:%02x:%02x\n",__FUNCTION__,__LINE__,hwaddr[0],hwaddr[1],hwaddr[2],hwaddr[3],hwaddr[4],hwaddr[5]);/*Zz*/
    }
    RELAY_DEBUG("Zz [fun=%s,%d],\n",__FUNCTION__,__LINE__);/*Zz*/
    /* Sanity check on MTU */
    strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],ifindex:%d\n",__FUNCTION__,__LINE__,ifr.ifr_ifindex);/*Zz*/
#else
    ...
    RELAY_DEBUG("Zz [fun=%s,%d],fd=%d\n",__FUNCTION__,__LINE__,fd);/*Zz*/
    return fd;
}
...
sendPacket(PPPoEConnection *conn, int sock, PPPoEPacket *pkt, int size)
{
    ...
    {
        sprintf(sourceMac, "%02x:%02x:%02x:%02x:%02x:%02x", 
                        pkt->ethHdr.h_source[0],
                    	pkt->ethHdr.h_source[1],
                        pkt->ethHdr.h_source[2],
                        pkt->ethHdr.h_source[3],
                        pkt->ethHdr.h_source[4],
                        pkt->ethHdr.h_source[5]);
        if (!strcmp(wanPortMac, sourceMac) )
        {/*transmit packet, send to pppoe server on wan*/
            txRelay++;
            txByte = txByte + size;
        }
        else
        {/*received packet, send to pppoe client on lan*/
            rxRelay++;
            rxByte = rxByte + size;
        }
    }
