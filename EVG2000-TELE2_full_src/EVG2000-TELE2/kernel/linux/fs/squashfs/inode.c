...
ERROR("1. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize); 
		goto skip_read;
	}
	...
	if (SQUASHFS_I(inode)->u.s1.fragment_start_block == SQUASHFS_INVALID_BLK
		...
		if (!(bytes = squashfs_read_data(inode->i_sb, msblk->read_page,
			...
			ERROR("Unable to read page, block %llx, size %x\n", block,
					bsize);
ERROR("2. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize); 
		...
		if ((fragment = get_cached_fragment(inode->i_sb,
			...
			ERROR("Unable to read page, block %llx, size %x\n",
					...
					u.s1.fragment_size);
ERROR("3. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize); 
			goto skip_read;
		}
...
static int squashfs_readpage4K(struct file *file, struct page *page)
{
	...
	if (!(block_list = kmalloc(SIZE, GFP_KERNEL))) {
		ERROR("Failed to allocate block_list\n");
ERROR("4. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize); 
		goto skip_read;
	}
...
ERROR("4. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize); 
		...
		else
		{
			ERROR("Unable to read page, block %llx, size %x\n",
					...
					SQUASHFS_I(inode)-> u.s1.fragment_size);
ERROR("5. Unable to read page (%s, %d), block %x, size %x\n", file->f_dentry->d_name.name, current? current->pid:-1, block, bsize);
		}
	}
