			...
			if (softirq_preemption) {
				...
				else
				{
					for (i = 0; i < MAX_SOFTIRQ; i++) {
						p = per_cpu(ksoftirqd, cpu)[i].tsk;
						param.sched_priority = softirq_prio[data->nr].prio;
						sched_setscheduler(p, softirq_prio[data->nr].type, &param);
						set_user_nice(p, softirq_prio[data->nr].nice);
					}
				}
			}
			else {
