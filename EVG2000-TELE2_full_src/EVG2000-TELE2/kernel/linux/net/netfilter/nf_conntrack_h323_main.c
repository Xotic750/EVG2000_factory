...
static char *h323_big_buffer = NULL;
#endif
...
#ifdef CONFIG_MIPS_BRCM
		/* This ALG won't change the packet size because
		 * it's binary encoded. */
		if (tcpdatalen > NF_ALG_BUFFER_SIZE) {
			h323_big_buffer = kmalloc(tcpdatalen, GFP_ATOMIC);
			if (!h323_big_buffer)
				goto clear_out;
...
#ifdef CONFIG_MIPS_BRCM
 	if (h323_big_buffer) {
		kfree(h323_big_buffer);
		h323_big_buffer = NULL;
	}
#endif
...
#ifdef CONFIG_MIPS_BRCM
	/* This ALG won't change the packet size because it's binary encoded. */
	if (*datalen > NF_ALG_BUFFER_SIZE) {
		h323_big_buffer = kmalloc(*datalen, GFP_ATOMIC);
		if (!h323_big_buffer)
			return NULL;
		return skb_header_pointer(*pskb, dataoff, *datalen,
					  h323_big_buffer);
	} else
#endif
...
static int __init nf_conntrack_h323_init(void)
{
	...
	h323_buffer = kmalloc(NF_ALG_BUFFER_SIZE, GFP_KERNEL);
#else
